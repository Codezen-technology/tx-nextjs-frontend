...
| QA-BLOGS-A7 | `R-SBLOG-1920-07` | "the FAQ section is not similar to the actual design"                                                    | all      | A     | STILL-BROKEN   | MANUAL-VISUAL                                                                                 | **Measured, not applied.** Seven deltas against `6015:127392`: container fill `secondary-50` at 50% alpha vs solid; square vs `rounded-lg`+border; divider `N30` #EBEDF1 vs `secondary-50`; question Open Sans **400**/`N500` vs 500/`N900`; icon **24px** vs 20px; answer panel outer-24 → inner `secondary-50` solid vs `m-4`+`secondary-100`; answer text 1.5/`N500` vs `leading-relaxed`/`N600`. Full table in `.context/figma/targets.md`. Not fixed here: `CourseFaq` also renders on the course and category pages, so applying a blog frame to it needs a scoping decision this row does not carry. |
...
**9 tests owed** — 16 filed by the reconciliation, four closed by `qa-homepage-nav-footer-rows` and three by `qa-single-blog-rows` — against an empty table before the reconciliation. The table was empty
