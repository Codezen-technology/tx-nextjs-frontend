...
| ------------------ | --------------------- | ----- | ---- | ------- | ------------------------------------ |
| Homepage           | `/`                   | AMBER | 0    | 3       | `qa-homepage-remaining-rows`         |
...
| QA-HOME-A5 | "the pound symbol… doesn't feel like a pound symbol"          | all       | A     | FIXED          | `e2e/design-fidelity.spec.ts > prices render in one loaded family, symbol and digits alike`                                                | Closed by verification — **the report and the row were both wrong**. The frame binds the price (`6089:107486`) to `Heading/Bold/H2` = **SUSE**, and the build already computes `SUSE, "SUSE Fallback"` at 1920 and 440; it never computed Open Sans, as this row claimed. Inter appears in `6013:89909` once, as `Text xs/Medium` (12px), bound to nothing in the pricing card — so no third webfont is loaded. The assertion pins the family _and_ that it is actually loaded, which is the condition under which a `£` would fall through to a different face from its digits. Evidence in `.context/figma/targets.md`. Separately measured and **not** filed: the frame's price is 32px, the build ships 24px.                                 |
...
_Empty — all three entries closed by the Homepage remainder slice._
