# upsell-banner Specification

## Purpose

Governs the membership upsell banner carried by Cart, Checkout and Order
Confirmation — one component with no per-page variant, so the offer reads the same
wherever a shopper meets it.
...
`cta_label` is not in `API_REFERENCE.md` and older plugin builds omit it, so the
button SHALL fall back to "Add to Cart" rather than rendering blank.

#### Scenario: CTA shows dynamic label

- **WHEN** the banner renders and the API response includes `cta_label`
- **THEN** the button displays the `cta_label` value from the API

#### Scenario: CTA falls back when the API omits a label

- **WHEN** the API response has no `cta_label`
- **THEN** the button displays "Add to Cart"
