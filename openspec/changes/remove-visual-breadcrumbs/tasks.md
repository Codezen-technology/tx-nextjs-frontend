...
- [x] 4.1 Confirm the diff leaves every `BreadcrumbList` JSON-LD block untouched —
...
- [x] 4.2 Confirm no `<nav>` remains whose contents are an ancestor trail: the only
      surviving navs should be pagination, header/footer, sidebars and tab strips
- [x] 4.3 Run `pnpm typecheck` (clean) and `pnpm lint` (0 errors)
- [x] 4.4 Run `pnpm test` and confirm the suite passes with no breadcrumb-related failures
