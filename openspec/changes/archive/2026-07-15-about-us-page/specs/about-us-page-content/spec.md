...
The rendered `/about` page SHALL present sections in this order: Breadcrumb, Trusted strip, Hero, three alternating commitment blocks, values grid, Team section — matching the Figma "About US" design (node `649:22654`). The previous implementation's closing stats section (learner counts, guarantee callouts) SHALL be removed, as it has no counterpart in the Figma design.

#### Scenario: Page render order

- **WHEN** `/about` is rendered
- **THEN** the sections appear in the order Breadcrumb, Trusted, Hero, commitment blocks, values grid, Team — and no stats/"Our journey" section is present
