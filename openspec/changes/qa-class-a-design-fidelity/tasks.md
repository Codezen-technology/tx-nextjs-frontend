...
- [x] 5.4 **Course Category** — all 3 rows `FIXED`. `A1`: the page never used the page grid at all — hero, course grid and Why Choose Us each rolled `mx-auto max-w-[1296px] px-4`, putting content at 16 while the header sat at 128; all four now use `container` · `A2`: band `3294:42433` is 480 around content ending at 374 → inset **106**, applied as `min-h-[350px] 2xl:py-[106px]` in place of a hard `height: 350` whose inset shrank as the title wrapped · `A3`: weight already 700 and now asserted, casing not a defect on this page's own frame evidence
- [x] 5.5 **All Courses** — both Class A rows `FIXED`. `A2`: band `3306:50115` is 320 around content ending at 208 → inset **112**, which the build already shipped — verify-and-close with an assertion added, no code · `A3`: all 17 h2 already 700 and now asserted, casing not a defect (the headings are WordPress category names). Also brought the page onto the page grid: hero, client and loading each restated `max-w-[1296px] px-4`, which `page-grid` forbids — no QA row, because the report marks this page's laptop width "Working Fine". `QA-COURSES-D1` (mobile) remains open as Class D
- [x] 5.6 **Checkout + Priority Support** — all 3 rows `FIXED`. The dropdown the report meant is the Gravity Form select both pages share (`forms/gravity-form.tsx`), which sat at `px-3` = **12** with a flush native arrow; now 16. `gf-fields/select-field.tsx` had its chevron at `right-3`, also 12, now `right-4`. The live-WP fallback was attempted and had nothing to measure — WP renders no equivalent dropdown — so the report's own explicit 16px is the recorded source. `CHECK-A2`: weight already 700 and casing matches the frame's own mix exactly (`6239:134664` "Payment method" is sentence case in the design too). Finding recorded but not applied — the frame's headings are 32px and the build ships 24px, which no row names and the report's Checkout section never raises
- [~] 5.7 **Contact + Cancellations** — the two rows this slice was scoped to do not exist in the report. Its Contact section files "section colors are not according to the design"; its Cancellations section files button-label contrast and icon colour; neither page has a hero-spacing item at any breakpoint, and both are marked "Working Fine" at laptop and mobile. Both A1 rows are `RECLASSIFIED` and the three real items are now filed: `QA-CONTACT-A2` (`MANUAL-VISUAL`, frame `3277:44993` is valid — the row's "No Figma ref" was also wrong), `QA-CANCEL-A2` (measured **4.42:1** against WCAG AA 4.5) and `QA-CANCEL-E1` (measured **1.49:1**, blocked on design). No code: the button colour is a site-wide token decision and the report names no replacement. **Third page where a row was generalised in during triage — see `targets.md`**
