        ...
        <>
            ...
            {items.map((item) => (
              <li key={item.key} className="flex items-center gap-3 px-4 py-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className="font-open-sans line-clamp-2 text-[13px] leading-snug font-medium text-neutral-800">
                    {item.name}
                  </p>
                  <p className="font-open-sans mt-0.5 text-[13px] text-neutral-500">
                    {fmt(item.price, sym)}
                  </p>
                </div>
                <button
          ...
          <div className="grid grid-cols-2 gap-2 px-4 pb-4">
            <Link
              href="/cart"
              onClick={onClose}
              className="font-open-sans flex h-10 items-center justify-center rounded bg-neutral-900 text-[13px] font-semibold text-white transition hover:bg-[#001733]"
            >
              View Cart
            </Link>
            <Link
